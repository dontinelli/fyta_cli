# coding: utf-8

from datetime import datetime

from .client import Client

PLANT_STATUS = {
    1: "too low",
    2: "low",
    3: "perfect",
    4: "high",
    5: "too high",
    }

class FytaConnector(object):
    def __init__(self, email, password, access_token = "", expiration = None):

        self.email = email
        self.password = password
        self.client = Client(email, password, access_token, expiration)


    async def test_connection(self) -> bool:

        return await self.client.test_connection()


    async def login(self) -> bool:
        login = await self.client.login()
        self.access_token = login["access_token"]
        self.expiration = login["expiration"]

        self.online = True

        return login


    async def update_data(self):
        self.plant_list = await self.client.get_plants()

        return self.plant_list


    async def update_all_plants(self):

        plants = {}

        for plant in plant_list:
            current_plant = await update_plant_data(plant["plant_id"])
            plants |= {plant["plant_id"], current_plant}

        self.plants = plants

        return plants


    async def update_plant_data(self, plant_id):

        plant_data = await self.client.get_plant_data(plant_id)

        current_plant = {}
        current_plant |= {"online", True}
        current_plant |= {"battery_status", plant_data["sensor"]["is_battery_low"]}
        current_plant |= {"plant_id", plant_data["plant"]["plant_id"]}
        current_plant |= {"name", plant_data["plant"]["nickname"]}
        current_plant |= {"scientific_name", plant_data["plant"]["scientific_name"]}
        current_plant |= {"status", plant_data["plant"]["status"]}
        current_plant |= {"temperature_status", plant_data["masurements"]["temperature"]["status"]}
        current_plant |= {"light_status", plant_data["masurements"]["light"]["status"]}
        current_plant |= {"moisture_status", plant_data["masurements"]["moisture"]["status"]}
        current_plant |= {"salinity_status", plant_data["masurements"]["salinity"]["status"]}
        current_plant |= {"ph", plant_data["masurements"]["ph"]["values"]["current"]}
        current_plant |= {"temperature", plant_data["masurements"]["temperature"]["values"]["current"]}
        current_plant |= {"light", plant_data["masurements"]["light"]["values"]["current"]}
        current_plant |= {"moisture", plant_data["masurements"]["moisture"]["values"]["current"]}
        current_plant |= {"salinity", plant_data["masurements"]["salinity"]["values"]["current"]}
        current_plant |= {"battery_level", plant_data["masurements"]["battery"]}
        current_plant |= {"last_updated", datetime.fromisoformat(plant_data["sensor"]["received_data_at"])}

        return current_plant


    @property
    def fyta_id(self) -> str:
        """ID for FYTA object"""
        return self.email

    @property
    def online(self) -> bool:
        """Online status"""
        return self.email

    @property
    def data(self) -> dict:
        """All FYTA plant data"""
        return self.data
